...
This repository contains a static reporting snapshot refreshed from the validated QTD weekly master through Wk12 on 21 Sep 2026. The canonical model covers 69 stores and 15 ARMs.

The QTD exit comparison normalizes cumulative Wk12 performance to a 13-week quarter. The source already excludes the known transaction `LOGTI/2627/3821` (quantity 46,000) from Aptronix Logix iPhone trade-in attachment, and the updater verifies that it is not deducted again. The loan-only Aptronix FRM Coimbatore row has zero activity and remains outside the canonical 69-store model.
...
- `qtd-data.test.mjs` reconciles the embedded Wk12 dataset, headline totals, coverage, QA exclusions, and source metadata.
